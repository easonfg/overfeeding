## testing plot when there is no combat correction just raw values
rm(list=ls())
...
tp4 = read.csv('logistic2_more_ctrl_batchCorrectinLM/4p.val__Median_more_ctrl_BatchCinLM.csv', row.names = 1,
               header = T, stringsAsFactors = F, sep=',', dec='.')
tp3 = read.csv('logistic2_more_ctrl_batchCorrectinLM/3p.val__Median_more_ctrl_BatchCinLM.csv', row.names = 1,
               header = T, stringsAsFactors = F, sep=',', dec='.')
tp2 = read.csv('logistic2_more_ctrl_batchCorrectinLM/2p.val__Median_more_ctrl_BatchCinLM.csv', row.names = 1,
               header = T, stringsAsFactors = F, sep=',', dec='.')

tp4$timepoint.p.val.P
tp4 = tp4[tp4$timepoint.p.val.P < 0.05,]
tp3 = tp3[tp3$timepoint.p.val.P < 0.05,]
tp2 = tp2[tp2$timepoint.p.val.P < 0.05,]
...
int.list = unique(all.sig)
# pdf(paste0("single_plot/intersecting_cyto.pdf"), onefile = TRUE)
pdf(paste0("single_plot/all.more.ctrl.abilm.pdf"), onefile = TRUE)
...
#### average of all cytokines
colmean.sig.cyto = bind_cols(cytof.data[which(annotations$timepoints == 1),int.list] %>% colMeans(),
          cytof.data[which(annotations$timepoints == 2),int.list] %>% colMeans(),
          cytof.data[which(annotations$timepoints == 3),int.list] %>% colMeans(),
          cytof.data[which(annotations$timepoints == 4),int.list] %>% colMeans()) %>%
  data.frame()
rownames(colmean.sig.cyto) = cytof.data[which(annotations$timepoints == 1),int.list] %>% colMeans() %>% names()

colmean.sig.cyto = bind_cols(cytof.data[which(annotations$timepoints == 1),] %>% colMeans(),
                             cytof.data[which(annotations$timepoints == 2),] %>% colMeans(),
                             cytof.data[which(annotations$timepoints == 3),] %>% colMeans(),
                             cytof.data[which(annotations$timepoints == 4),] %>% colMeans()) %>%
  data.frame()
rownames(colmean.sig.cyto) = cytof.data[which(annotations$timepoints == 1),] %>% colMeans() %>% names()

library(tibble)
colmean.sig.cyto = colmean.sig.cyto %>% rownames_to_column()
colmean.sig.cyto %>% head()
colmean.sig.cyto %>% str()
colnames(colmean.sig.cyto) = c('cytokines', paste0('tp',seq(1:4)))
library(tidyr)
data_long <- gather(colmean.sig.cyto, timepoints, values, c(2:5), factor_key=TRUE)
data_long$cytokines = factor(data_long$cytokines)
# data_long$timepoints <- gsub('\\.\\.\\.', '', data_long$timepoints) %>% factor()
head(data_long)
str(data_long)
data_long %>%
  ggplot(aes(x = timepoints, y = values)) +
  geom_line(aes(group=cytokines)) +
  geom_point()
  # print()
                              
## ones that are increasing
head(colmean.sig.cyto)
colmean.sig.cyto$tp2 - colmean.sig.cyto$tp1 > 0

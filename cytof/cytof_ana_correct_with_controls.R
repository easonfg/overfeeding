...
## batch correction
cytof.data[(c(which(annotations$batches == 'batch2'))),] = sweep(cytof.data %>% slice(c(which(annotations$batches == 'batch2'))) ,
                                                                 2, ctrl_correct[,1], `/`)
cytof.data[(c(which(annotations$batches == 'batch3'))),] = sweep(cytof.data %>% slice(c(which(annotations$batches == 'batch3'))) ,
                                                                 2, ctrl_correct[,2], `/`)
cytof.data[(c(which(annotations$batches == 'batch4'))),] = sweep(cytof.data %>% slice(c(which(annotations$batches == 'batch4'))) ,
                                                                 2, ctrl_correct[,3], `/`)
cytof.data[(c(which(annotations$batches == 'batch5'))),] = sweep(cytof.data %>% slice(c(which(annotations$batches == 'batch5'))) ,
                                                                 2, ctrl_correct[,4], `/`)


pca.cytof.combat.res = prcomp(cytof.data,  scale = T)
library("factoextra")
...
pipe = function(timepoint, in.group, datatype, pval) {
  ...
  if (nchar(in.group) > 0){
    data1 = data.frame(cytof.data) %>% slice(c(which(annotations$timepoints == 1 & annotations$groups == in.group)))
    data2 = data.frame(cytof.data) %>% slice(c(which(annotations$timepoints == timepoint & annotations$groups == in.group)))
  }else{
    data1 = data.frame(cytof.data) %>% slice(c(which(annotations$timepoints == 1)))
    data2 = data.frame(cytof.data) %>% slice(c(which(annotations$timepoints == timepoint)))
  }
  ...
  jpeg(paste('volcano/ctrl_correct/', datatype, '/', timepoint, pval, '_', in.group, '_', datatype, '_ctrl_corrected.jpeg', sep = ''),

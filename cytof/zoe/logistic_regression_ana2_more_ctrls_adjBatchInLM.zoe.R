...
library(dplyr)
require(aod)
cytof.data = read.csv('overfeeding_Blood_Screen_Cytof_171213.xlsx - NewRAW.csv', row.names = 1,
...
pipe = function(timepoint1, timepoint2, in.group, datatype, pval) {
  ...
  write.csv(res[res[,2]  < 0.05,], paste('zoe/', timepoint1, '_', timepoint2, '_', in.group, '_', datatype, '_more_ctrl_BatchCinLM.csv', sep = ''))

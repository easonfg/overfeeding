...
pipe = function(timepoint, in.group, datatype, pval) {
  ...
  if (nchar(in.group) > 0){
    ...
    sub.annotate1 = annotations %>% slice(c(which(annotations$timepoints == 1 & annotations$groups == in.group)))
    sub.annotate2 = annotations %>% slice(c(which(annotations$timepoints == timepoint & annotations$groups == in.group)))
  }else{
    ...
    sub.annotate1 = annotations %>% slice(c(which(annotations$timepoints == 1)))
    sub.annotate2 = annotations %>% slice(c(which(annotations$timepoints == timepoint)))
  }
  ...
  res = sapply(colnames(logitstic.data[,1:(ncol(logitstic.data)-3)]), function(x){
    ...
    pair.model = lm(as.formula(paste(x, '~timepoint+batch')), data = logitstic.data)
    ...
    wt.res.batch = wald.test(b=coef(pair.model), Sigma=vcov(pair.model), Terms=c(3))
    # sum.model = summary(pair.model)
    c(Estimates = unname(pair.model$coefficients[2]),
  ...
  res = res[order(res$timepoint.padj),]
  # head(res) %>% View()
  
  write.csv(res[res$timepoint.p.val.P < 0.05,], paste('logistic2_batchCorrectinLM/', timepoint, pval, '_', in.group, '_', datatype, '_BatchCinLM.csv', sep = ''))

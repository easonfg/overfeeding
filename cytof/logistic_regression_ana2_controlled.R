...
pipe = function(timepoint, in.group, datatype, pval) {
  ...
  res = sapply(colnames(logitstic.data[,1:(ncol(logitstic.data)-2)]), function(x){
    print(x)
    # pair.model = glm(as.formula(paste(x, '~timepoint')),
    #                  data=logitstic.data, family=binomial(link="logit"))
    pair.model = lm(as.formula(paste(x, '~timepoint + group')), data = logitstic.data)
  ...
  write.csv(res[res$timepoint.p.val.P < 0.05,], paste('logistic2_controlled/', timepoint, pval, '_', in.group, '_', datatype, '_controlled.csv', sep = ''))
